...
class neutron_hyper_v (
  ...
  $log_dir                     = $::neutron_hyper_v::params::log_dir,
  $state_path                  = $::neutron_hyper_v::params::state_path,
  $lock_path                   = $::neutron_hyper_v::params::lock_path,
  $neutron_version             = 'installed',
  $neutron_repository          = 'git+https://github.com/openstack/neutron',
  $neutron_source              = false,
) inherits neutron_hyper_v::params {


  # Package['neutron'] -> Nova_Admin_Tenant_Id_Setter<||>

  # all hyperv_neutron_config resources should be applied
  # before the file resource for neutron.conf is managed
  # and before the post config resource
  Class['neutron_hyper_v::common'] -> Hyperv_neutron_config<| |> -> File[$::neutron_hyper_v::params::confdir]
  Hyperv_neutron_config<| |> ~> Exec['post-neutron_config']

  class { 'neutron_hyper_v::dependencies': }

  class { 'neutron_hyper_v::utilities': }

  anchor { 'neutron-start': }

  class { 'neutron_hyper_v::common':
    require => Anchor['neutron-start']
  }

  file { $::neutron_hyper_v::params::confdir:
    mode => '0650',
  }
  ...
  package { 'neutron':
    ensure   => $neutron_version,
    provider => pip,
    notify   => Exec['post-neutron_config'],
    require  => [Class['neutron_hyper_v::utilities'], Class['neutron_hyper_v::dependencies']],
  }

  file { 'C:/OpenStack/etc/neutron/policy.json':
    ensure  => file,
    mode    => 0770,
    source  => "puppet:///modules/neutron_hyper_v/policy.json",
    notify  => Exec['post-neutron_config'],
    require => Class['neutron_hyper_v::common'],
  }

  hyperv_neutron_config {'DEFAULT/policy_file': value => 'C:\OpenStack\etc\neutron\policy.json';}
  ...
  if $service_plugins {
    if is_array($service_plugins) {
      hyperv_neutron_config { 'DEFAULT/service_plugins': value => join($service_plugins, ',') }
    } else {
  ...
  if $rpc_backend == 'neutron.openstack.common.rpc.impl_kombu' {
    ...
    if $rabbit_hosts {
      hyperv_neutron_config { 'DEFAULT/rabbit_hosts':     value  => join($rabbit_hosts, ',') }
      hyperv_neutron_config { 'DEFAULT/rabbit_ha_queues': value  => true }
    } else  {
      hyperv_neutron_config { 'DEFAULT/rabbit_host':      value => $rabbit_host }
      hyperv_neutron_config { 'DEFAULT/rabbit_port':      value => $rabbit_port }
      hyperv_neutron_config { 'DEFAULT/rabbit_hosts':     value => "${rabbit_host}:${rabbit_port}" }
      hyperv_neutron_config { 'DEFAULT/rabbit_ha_queues': value => false }
    }
    ...
    if $rabbit_use_ssl {
      ...
      if $kombu_ssl_ca_certs {
        hyperv_neutron_config { 'DEFAULT/kombu_ssl_ca_certs': value => $kombu_ssl_ca_certs; }
      } else {
        hyperv_neutron_config { 'DEFAULT/kombu_ssl_ca_certs': ensure => absent; }
      }
      ...
      if $kombu_ssl_version {
        hyperv_neutron_config { 'DEFAULT/kombu_ssl_version':  value => $kombu_ssl_version; }
      } else {
        hyperv_neutron_config { 'DEFAULT/kombu_ssl_version':  ensure => absent; }
      }
  ...
  # SSL Options
  hyperv_neutron_config { 'DEFAULT/use_ssl' : value => $use_ssl; }
  if $use_ssl {
    hyperv_neutron_config {
    ...
    if $ca_file {
      hyperv_neutron_config { 'DEFAULT/ssl_ca_file'   : value => $ca_file; }
    } else {
      hyperv_neutron_config { 'DEFAULT/ssl_ca_file'   : ensure => absent; }
    }
  } else {
    hyperv_neutron_config {
  ...
  exec { 'post-neutron_config':
    command     => 'cmd.exe /c echo "Neutron config has changed"',
    refreshonly => true,
    path        => $::path,
  }
